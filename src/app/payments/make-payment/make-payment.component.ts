...
export class MakePaymentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.paymentService.getPricingTier(type).subscribe(
      ...
      (err) => {
        this.logger.debug('make-payment.component', 'getPricingTier', err);
  ...
  _spinUpMerchant(pricingTier: any) {
    this.scriptService
      ...
      .then(() => {
        this.handler = StripeCheckout.configure({
          ...
          token: (token: { id: any }) => {
            ...
            this.paymentService
              .processPayment(token.id, this.chargingAmount, pricingTier.type)
              .subscribe(
                (r) => {
                  if (r) {
                ...
                (error) => {
                  ...
                  this.errorText =
                    ...
                    `<a href="${environment.helpUrl}"> PodNoms Support</a> so we can track your payment`;
                }
              );
          },
        });
      ...
      .catch((err) =>
        this.logger.error('make-payment.component', 'Error loading stripe', err)
      );
  }
  handlePayment() {
