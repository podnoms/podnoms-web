...
export class EntryDataService extends DefaultDataService<PodcastEntry> {
    ...
    getWithQuery(
        ...
        return this.execute(
            'GET',
            `${this.entitiesUrl}all/${queryParams['podcastSlug']}`,
            undefined,
            params
        );
    }
    getAllEntriesForUser(): Observable<PodcastEntry[]> {
        return this.http.get<PodcastEntry[]>(
            environment.apiHost + '/entry/users/'
        );
    }
    getEntry(id: string): Observable<PodcastEntry> {
        return this.http.get<PodcastEntry>(
            environment.apiHost + '/entry/' + id
        );
    }
    getEntries(slug: string): Observable<PodcastEntry[]> {
        return this.http.get<PodcastEntry[]>(
            environment.apiHost + '/entry/all/' + slug
        );
    }
    ...
    reSubmitEntry(entry: PodcastEntry): Observable<PodcastEntry> {
        return this.http.post<PodcastEntry>(
            environment.apiHost + '/entry/resubmit',
            entry
        );
    }
    //#endregion
