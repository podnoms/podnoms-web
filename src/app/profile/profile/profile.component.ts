...
export class ProfileComponent
  ...
  constructor(
    ...
    this.route.params.subscribe((param) => {
      if (param && param['view']) {
        this.logger.debug('profile.component', 'fragmentChanged', param);
        this.activeTab = param['view'];
      }
    });
    this.logger.debug('profile.component', 'ctor', 'activeTab', this.activeTab);
    this.searchTerm$
      .pipe(
        debounceTime(400),
        distinctUntilChanged(),
    ...
    this.profileDataService.updateProfile(profile).subscribe((p) => {
      if (this.imageControl.imageChanged) {
        this.imageControl.commitImage(p.id, 'profile').subscribe(
          (r) => {
            profile.profileImageUrl = `${r.profileImageUrl}?v=${UUID.UUID()}`;
