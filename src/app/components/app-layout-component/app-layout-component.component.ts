  ...
  ngOnInit() {
    ...
    if (environment.production || false) {
      this.utilityService.checkForApiServer().subscribe(
        ...
        (err) => {
          this.logger.error('home.component', 'checkForApiServer', err);
    ...
    this.authService.authNavStatus$.subscribe((r) => {
      if (r) {
        ...
        this.signalr
          ...
          .then((listener) => {
            listener
              .on<SiteUpdateMessage>('userupdates', 'site-notices')
              .subscribe((result: SiteUpdateMessage) => {
                ...
                this.alertService.info(result.title, result.message, {
  ...
  _bootstrapUpdates(isReady: boolean) {
    ...
    profile$.subscribe((p) => {
      ...
      if (p && environment.production) {
        ...
        this.logger.debug('app.component', 'requesting subscription', p);
        this.swPush
          .requestSubscription({
            serverPublicKey: environment.vapidPublicKey,
          })
          .then((s) => {
            this.logger.debug('app.component', 'requested subscription', s);
            this.logger.debug('app.component', 'subscribing on server', p);
            this.pushRegistrationService.addSubscriber(s.toJSON()).subscribe(
              (r) => {
                this.logger.debug('app.component', 'push request succeeded', r);
                this.swPush.messages.subscribe((message) => {
                  this.logger.debug('app.component', 'Push message', message);
                });
              },
              (err) =>
                this.logger.error(
        ...
        this.logger.debug('app.component', 'Unable to load profile from store');
      }
    });
  }
  _unsubscribe() {
